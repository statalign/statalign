		...
		// Add helices...
		 for (int i=0;i<r._helices.size();i++) { 
			 ...
			 { 
				 ...
				 int a = (h.pos5+j-1); 
				 int b = (h.pos3-j-1); 
				 // Base pair type in helices defaults to canonical
			...
			Hashtable<Integer,ArrayList<BPTemp>> index2BPs = new Hashtable<Integer,ArrayList<BPTemp>>();
			for(BPTemp msbp :  r._structure)
			{
				int i = msbp.pos5; 
				if (!index2BPs.containsKey(i))
				{  index2BPs.put(i,new ArrayList<BPTemp>()); }
