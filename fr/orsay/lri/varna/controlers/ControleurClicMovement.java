...
public class ControleurClicMovement implements MouseListener,
	...
	public void mousePressed(MouseEvent arg0) 
	{
		...
		if (button1 && !ctrl && !alt && !shift)
		{
			if (_vp.isModifiable()) 
			{
				...
				{
					_selectedBase = getNearestBase(arg0,false,false);
	...
	public void mouseDragged(MouseEvent me) {
		...
		else if (_currentState == MouseStates.CREATE_BP)
		{
			if (_initialPoint.distance(me.getX(),me.getY())>HYSTERESIS_DISTANCE)
			{
				ModeleBase newSelectedBase = getNearestBase(me,false,false);
		...
		if (arg0.getButton() == MouseEvent.BUTTON1)
		{
			...
			else if (_currentState == MouseStates.SELECT_ELEMENT)
			{
				if (_vp.getRealCoords() != null
						...
						&& _vp.getRNA().get_listeBases().size() != 0) 
					{
						int selectedIndex = getNearestBaseIndex(arg0,false,false);
						if (selectedIndex !=-1)
						{ 
			...
			else if (_currentState == MouseStates.CREATE_BP)
			{
				if (_initialPoint.distance(arg0.getX(),arg0.getY())>HYSTERESIS_DISTANCE)
				{
					...
					if (selectedIndex>=0)
					{
						ModeleBase mb = getNearestBase(arg0,false,false);
						ModeleBase mborig = _selectedBase;
	...
	private ModeleBase getNearestBase(MouseEvent arg0) {
		return getNearestBase(arg0, false ,false);
	}
	...
	public void mouseMoved(MouseEvent arg0) {
		_selectedBase = getNearestBase(arg0);
		TextAnnotation selectedAnnotation = getNearestAnnotation(arg0);
