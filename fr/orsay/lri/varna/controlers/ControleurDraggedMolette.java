...
public class ControleurDraggedMolette implements MouseListener,
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (_rightButtonClick) {
			// la position du curseur est enregistrée
			_apres = e.getPoint();
			// le vecteur direction est crée avec la difference du point avant
			// et après
			_direction = new Point(_apres.x - _avant.x, _apres.y - _avant.y);
			// la translation est mise a jour sur le VARNAPanel
			_vp.setTranslation(new Point(_vp.getTranslation().x + _direction.x,
					_vp.getTranslation().y + _direction.y));
			// le point avant est mise à jour à la valeur du point d'après
			_avant = _apres;
